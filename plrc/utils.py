...
def GetAuthProfile(jso, profile_name="local", args=None):
    ...
    if "profiles" in jso:
        for item in jso["profiles"]:
            if item["name"] == profile_name:
                ret = item
    elif "plrc_db" in jso and "oidc" in jso:
        ret = jso
    ...
    if args:
        ret['plrc_db']['user']      = ret['plrc_db']['user']        if not args.dbuser else args.dbuser
        ret['plrc_db']['sid']       = ret['plrc_db']['sid']         if not args.dbsid  else args.dbsid
        ret['plrc_db']['password']  = ret['plrc_db']['password']    if not args.dbpswd else args.dbpswd
        ret['plrc_db']['host']      = ret['plrc_db']['host']        if not args.dbhost else args.dbhost
        ret['plrc_db']['port']      = ret['plrc_db']['port']        if not args.dbport else args.dbport
